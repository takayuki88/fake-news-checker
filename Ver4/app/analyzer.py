...
Ver4 はローカル一次判定に加えて、OpenAI による一次レビューと
Gemini による根拠確認を組み合わせ、最後に `AnalysisResult` へ整えます。
"""
...
def derive_public_verdict(
    ...
    material_counterevidence_review = any(
        isinstance(review, dict)
        and normalize_evidence_verdict(review.get("verdict")) == "反証あり"
        and is_material_counterevidence_review(review)
        for review in claim_reviews
    )
    ...
    if claim_mode:
        ...
        if overall_verdict == "反証あり":
            ...
            if mostly_accurate_minor_counterevidence:
                return "ほぼ正確"
            if partially_supported_counterevidence and risk_score < 58 and not material_counterevidence_review:
                return "ほぼ正確"
...
def build_gemini_evidence_prompt(page: ResolvedPage, seed: dict[str, Any], settings: Settings) -> str:
    ...
    if settings.gemini_style_review_enabled:
        style_task_text = (
            "3. 本文の書き振りだけを見て、煽り・断定・感情誘導・文脈不足の強さを採点する\n"
...
役割は、本文から抽出した主張候補を外部根拠と照合して整理することです{("。加えて本文の書き振りを補助評価してください" if settings.gemini_style_review_enabled else "")}。
 GPT などによる一次判定の補助情報は参考にして構いませんが、最終的な外部根拠比較は google_search で確認した公開情報を優先してください。
 {style_task_text}
 {mode_instruction}
 必要に応じて google_search を使い、公開ウェブ上の一次ソース、公的機関、主要報道機関、ファクトチェック記事を優先して確認してください。
{metadata_guidance}
...
def build_prompt(page: ResolvedPage, seed: dict[str, Any], settings: Settings) -> str:
    """Gemini に渡すプロンプトを作る。Ver4 では根拠確認用プロンプトへ委譲する。"""
...
def merge_evidence_overview(seed: dict[str, Any], llm_bundle: dict[str, Any] | None, settings: Settings) -> dict[str, Any]:
    ...
    overview["assessment_model"] = f"{settings.gemini_model} + google_search"
...
def has_positive_claim_review_nuance(claim_reviews: list[dict[str, Any]]) -> bool:
    for review in claim_reviews:
        ...
        if re.search(r"\d", claim_text) and re.search(r"\d", reason_text):
            claim_allows_approximation = any(hint in claim_text for hint in ("約", "およそ", "ほど", "程度", "前後"))
            if any(hint in reason_text for hint in MOSTLY_ACCURATE_NUMERIC_UNCERTAINTY_HINTS) and not claim_allows_approximation:
                return True
            if any(hint in reason_text for hint in ("異なります", "異なる", "ではなく", "ではありません")):
                return True
        ...
        if any(hint in claim_text for hint in ("移った", "移行した", "移行した。")) and "その後" in reason_text:
            if any(hint in reason_text for hint in ("王政復古の大号令", "を経て")):
                return True
        if "懸賞金" in claim_text and "かけた" in claim_text and "引き上げ" in reason_text:
            return True
        if any(hint in claim_text for hint in ("作った", "作成した")):
            if "修正を加えて制定" in reason_text or ("修正を加え" in reason_text and "強い影響下" in reason_text):
                return True
            if (
                any(hint in reason_text for hint in ("GHQ草案", "原案", "草案"))
                and any(hint in reason_text for hint in ("日本政府", "帝国議会", "審議", "公布", "施行"))
                and any(hint in reason_text for hint in ("基に", "経て", "受け入れ"))
            ):
                return True
    return False
    ...
    for review in claim_reviews:
        ...
        for token in claim_tokens:
            for candidate in reason_tokens:
                ...
                if (
                    token in claim_text
                    and candidate in claim_text
                    and token in reason_text
                    and candidate in reason_text
                ):
                    continue
                if "代目" in token or "代目" in candidate:
                    continue
...
def has_materially_mixed_counterevidence(claim_reviews: list[dict[str, Any]]) -> bool:
    ...
    for review in claim_reviews:
        ...
        is_iraq_war_causal_mix = (
            ...
            and any(hint in reason_text for hint in ("匿ったという", "直接的な関連", "関連性", "協力関係", "無関係"))
        )
        ...
        is_everest_first_ascent_mix = (
            "エベレスト" in claim_text
            and "世界最高峰" in claim_text
            and any(hint in claim_text for hint in ("最初に登頂", "初登頂"))
            and any(hint in reason_text for hint in ("世界最高峰であることは正しい", "世界最高峰であることは"))
            and "エドモンド・ヒラリー" in reason_text
            and "テンジン・ノルゲイ" in reason_text
            and "植村直己" in reason_text
            and any(hint in reason_text for hint in ("一部誤り", "日本人初", "誤り"))
        )
        if is_everest_first_ascent_mix:
            return True
...
def has_mostly_accurate_minor_counterevidence(claim_reviews: list[dict[str, Any]]) -> bool:
    ...
    positive_text = " ".join(
        f"{review.get('claim') or ''} {review.get('reason') or ''}"
        for review in claim_reviews
        if isinstance(review, dict) and normalize_evidence_verdict(review.get("verdict")) == "概ね整合"
    )
    counter_text = " ".join(
        f"{review.get('claim') or ''} {review.get('reason') or ''}"
        for review in claim_reviews
    ...
    if (
        "イチロー" in positive_text
        and "200安打" in positive_text
        and "11回" in positive_text
        and any(hint in counter_text for hint in ("1回は日本プロ野球", "NPB", "日本プロ野球"))
        and any(hint in counter_text for hint in ("MLB", "メジャーリーグ"))
        and "10回" in counter_text
    ):
        return True
    ...
    for review in claim_reviews:
        ...
        if "標高" in claim_text and "標高" in reason_text and has_close_value_pair(
            ...
            return True
        if any(hint in claim_text for hint in ("本塁打", "ホームラン")) and "本" in reason_text and has_close_value_pair(
            claim_text, reason_text, absolute=1.0, relative=0.01
        ):
            return True
        is_supported_split_league_count_drift = (
            "イチロー" in claim_text
            and "200安打" in claim_text
            and "11回" in claim_text
            and any(hint in reason_text for hint in ("1回は日本プロ野球", "NPB", "日本プロ野球"))
            and any(hint in reason_text for hint in ("MLB", "メジャーリーグ"))
            and "10回" in reason_text
        )
        if is_supported_split_league_count_drift:
            return True
        ...
        is_supported_ev_range_drift = (
            ...
            and any(hint in reason_text for hint in ("発売", "開発", "記録", "完成"))
            and any(hint in reason_text for hint in ("走行距離", "1充電", "充電"))
            and any(hint in reason_text for hint in ("確認できません", "確認できない", "とは異な", "ではありません"))
        ...
        is_supported_footage_colorization_drift = (
            any(hint in claim_text for hint in ("映像", "カラー映像"))
            and any(hint in reason_text for hint in ("映像は存在", "保存"))
            and any(hint in reason_text.lower() for hint in ("ai", "着色", "カラー化"))
            and any(hint in reason_text for hint in ("オリジナルは白黒", "白黒"))
        )
        if is_supported_footage_colorization_drift:
            return True
...
def has_strong_false_counterevidence(claim_reviews: list[dict[str, Any]]) -> bool:
    for review in claim_reviews:
        ...
        has_saigo_seinan_government_win = any(
            hint in reason_text for hint in ("明治政府軍", "新政府軍", "政府軍", "明治政府（新政府軍）")
        )
        has_saigo_seinan_loss = any(hint in reason_text for hint in ("敗北", "鎮圧", "自刃", "自決"))
        is_saigo_seinan_victory_false = (
            not has_compound_structure
            and all(hint in claim_text for hint in STRONG_FALSE_SAIGO_SEINAN_VICTORY_CLAIM_HINTS)
            and has_saigo_seinan_government_win
            and has_saigo_seinan_loss
        )
        if (
            is_conspiracy_false
...
def combine_result(
    ...
    """ローカル一次判定、OpenAI一次レビュー、Gemini根拠確認を合成して最終判定を作る。"""
