...
def build_score_calculation(payload: dict[str, Any], public_verdict: str, attention_score: int | None) -> ScoreCalculation:
    ...
    if isinstance(heuristic_risk_score, int):
        steps.append(
            ScoreCalculationStep(
                label="外部根拠比較の補正",
                expression=f"{heuristic_risk_score}% {format_signed(evidence_delta)}",
...
def has_strong_false_counterevidence(claim_reviews: list[dict[str, Any]]) -> bool:
    for review in claim_reviews:
        ...
        if normalize_evidence_verdict(review.get("verdict")) != "反証あり":
            continue
        claim_text = str(review.get("claim") or "").strip()
        reason_text = str(review.get("reason") or "").strip().lower()
        if not claim_text or not reason_text:
            continue
        if "ため" in claim_text:
            continue
        is_conspiracy_false = "ケムトレイル" in claim_text and any(
            hint in reason_text for hint in STRONG_FALSE_CONSPIRACY_HINTS
        )
        ...
        is_fake_image_false = any(hint in reason_text for hint in ("画像", "写真")) and any(
            hint in reason_text for hint in STRONG_FALSE_FAKE_IMAGE_HINTS
        )
        if (
            is_conspiracy_false
...
def build_model_used(style_overview: dict[str, Any], has_llm_output: bool) -> str:
    if not has_llm_output:
        return "heuristic-fallback"
    if str(style_overview.get("status") or "") == "Gemini書き振り評価済み":
        return "heuristic+gemini-evidence+gemini-style"
    return "heuristic+gemini-evidence"
...
def combine_result(page: ResolvedPage, seed: dict[str, Any], llm_bundle: dict[str, Any] | None, settings: Settings) -> AnalysisResult:
    ...
    status = build_result_status(seed["status"], overall_verdict, confidence_score, seed.get("source_profile", {}), style_overview)
    summary = build_result_summary(seed["summary"], evidence_overview)
    labels = merge_result_labels(
        seed["labels"],
        evidence_labels,
        overall_verdict=overall_verdict,
        claim_mode=bool(seed.get("source_profile", {}).get("claim_mode")) or is_claim_mode(page),
    )
    ...
    internal_payload = dict(
        ...
        caution_level=label_for_score(risk_score),
        model_used=build_model_used(style_overview, True),
        source_snapshot=build_source_snapshot(page),
