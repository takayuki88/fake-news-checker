...
リポジトリ直下の `Ver1` から `Ver5` は試行錯誤と比較検証の履歴であり、採用担当者向けの主な確認対象はこの `portfolio/` です。
...
5ラベル各20件、合計100件のbalanced datasetで評価しました。
...
明確な正誤や判断保留は安定していました。
一方で、「正確」「ほぼ正確」「不正確」の境界に誤分類が残り、次の改善対象が明確になりました。
...
- 100件の評価データでAccuracy 90.0%、Macro F1 89.9%まで確認したこと
...
- [評価JSON](evaluation_outputs/20260427-0304/eval_real_article_dataset_v2_use_gpt_gemini.json)
- [予測結果JSON](evaluation_outputs/20260427-0304/predictions_real_article_dataset_v2_use_gpt_gemini.json)
