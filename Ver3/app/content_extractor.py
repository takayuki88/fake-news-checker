...
from .models import ResolvedPage, TimingOverview, TimingStage
...
META_FALLBACK_DOMAIN_SUFFIXES = [
    "x.com",
    "twitter.com",
    "threads.net",
]
X_OEMBED_DOMAIN_SUFFIXES = [
    "x.com",
    "twitter.com",
]
...
def build_timing_overview(stages: list[TimingStage]) -> TimingOverview:
    return TimingOverview(total_ms=sum(stage.duration_ms for stage in stages), stages=stages)


def default_policy_research_result() -> tuple[bool, str | None, str | None, str | None, str | None, list[str]]:
    return (
        True,
        None,
        "robots.txt優先(既定)",
        "既定設定では規約候補ページ探索を行わず、hard block と robots.txt を優先して判定しました。",
        None,
        [],
    )
...
def is_meta_fallback_url(url: str) -> bool:
    parsed = urlparse(url)
    hostname = (parsed.hostname or "").lower()
    path = parsed.path or ""
    return (
        any(hostname == suffix or hostname.endswith(f".{suffix}") for suffix in META_FALLBACK_DOMAIN_SUFFIXES)
        and "/status/" in path
    )


def build_meta_fallback_text(soup: BeautifulSoup) -> str:
    title = infer_title(soup)
...
def is_x_oembed_url(url: str) -> bool:
    parsed = urlparse(url)
    hostname = (parsed.hostname or "").lower()
    path = parsed.path or ""
    return (
        any(hostname == suffix or hostname.endswith(f".{suffix}") for suffix in X_OEMBED_DOMAIN_SUFFIXES)
        and "/status/" in path
    )


async def build_x_oembed_page(
    url: str,
    settings: Settings,
    ...
    oembed_url = f"https://publish.twitter.com/oembed?omit_script=1&url={quote_plus(url)}"
    headers = {
        "User-Agent": settings.request_user_agent,
    ...
    soup = BeautifulSoup(html, "html.parser")
    blockquote = soup.select_one("blockquote.twitter-tweet")
    if not blockquote:
        raise ValueError("X投稿の埋め込み情報を取得できませんでした。本文を手入力してください。")

    text_node = blockquote.select_one("p")
    analysis_text = normalize_whitespace(text_node.get_text(" ", strip=True) if text_node else "")
    if len(analysis_text) < settings.min_text_chars:
        raise ValueError("X投稿の本文を十分に取得できませんでした。本文を手入力してください。")

    links = blockquote.select("a[href]")
    published_at = normalize_whitespace(links[-1].get_text(" ", strip=True)) if links else None
    author_name = normalize_whitespace(str(payload.get("author_name") or "")) or None
    title = f"{author_name} の投稿" if author_name else "X の投稿"
    timestamp_fields = build_analysis_timestamp_fields(settings)
    note = (
        "投稿ページ本文を直接抽出できなかったため、X の公開埋め込み情報から投稿文を取得しました。"
        " JavaScript描画中心の投稿ページ向けのフォールバックです。"
    )
    if policy_note:
    ...
    return ResolvedPage(
        ...
        paragraph_count=max(1, sentence_count(analysis_text)),
        heading_count=1,
        extraction_score=52,
    ...
    async with httpx.AsyncClient(
        ...
        if policy_hostname != hostname:
            ...
            if resolved_robots_text and robots_disallow_path(resolved_robots_text, policy_path, settings.robots_user_agent):
                return False, MANUAL_PASTE_MESSAGE, None, None, None, []
            if not settings.strict_policy_research:
                return default_policy_research_result()
...
def parse_html_to_page(
    ...
    if not candidates:
        if is_meta_fallback_url(url):
            analysis_text = limit_text(build_meta_fallback_text(soup), settings.max_fetch_chars)
            if len(analysis_text) >= settings.min_text_chars:
                used_meta_fallback = True
                note = (
                    "URLから本文候補を十分に抽出できなかったため、メタ情報を使って判定対象文を組み立てました。"
                    " JavaScript描画中心の投稿ページ向けのフォールバックです。"
                )
                if policy_note:
                    note = f"{note} {policy_note}"
    ...
    if len(analysis_text) < settings.min_auto_extract_chars:
        if is_meta_fallback_url(url):
            fallback_text = limit_text(build_meta_fallback_text(soup), settings.max_fetch_chars)
            if len(fallback_text) >= settings.min_text_chars:
                analysis_text = fallback_text
                used_meta_fallback = True
                best = {
                    "label": f"{best['label']}+meta",
                    "reference_link_count": max(1, best["reference_link_count"]),
                    "paragraph_count": max(best["paragraph_count"], sentence_count(analysis_text)),
                    "heading_count": max(best["heading_count"], 1),
                    "score": min(best["score"], 280),
                }
            else:
    ...
    if description and not used_meta_fallback:
...
async def resolve_page_input(
    ...
    if cleaned_text:
        ...
        timing_overview = build_timing_overview(
            [
                TimingStage(
                    key="manual_text_prepare",
                    label="入力本文の整形",
                    duration_ms=elapsed_ms(overall_started_at),
                    note="手入力本文をそのまま解析対象にしました。",
                )
            ]
        )
        return (
        ...
        try:
            ...
            if skip_policy_check:
                ...
                stages.append(
                    TimingStage(
                        ...
                        note="規約確認をスキップして本文取得に進みました。",
                    )
                )
                parse_started_at = time.perf_counter()
                try:
                    page = parse_html_to_page(
                        html,
                        final_url,
                        settings,
                    ...
                    if not is_x_oembed_url(final_url):
                        raise
                    page = await build_x_oembed_page(
                        final_url,
                        settings,
                        policy_status="開発用スキップ",
                        policy_note="開発用チェックボックスにより、hard block / robots.txt / 規約候補ページ確認をスキップしました。",
                ...
                stages.append(
                    TimingStage(
                        key="page_parse",
                        label="本文抽出",
                        duration_ms=elapsed_ms(parse_started_at),
                        note="HTML から本文候補を抽出しました。",
                    )
                )
                page.timing_overview = build_timing_overview(stages)
                return page, None
            ...
            stages.append(
                TimingStage(
                    key="policy_check",
                    label="規約確認",
                    duration_ms=elapsed_ms(policy_started_at),
                    note=policy_status or "robots.txt と規約候補ページを確認しました。",
                )
            )
