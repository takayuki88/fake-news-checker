...
このガイドは、Ver5 のコードを初めて読む人向けの短い地図です。
細かい実装を追う前に、まず「どのファイルが何を担当しているか」をつかむために使ってください。

Ver5 は Ver3 をコピーして作った実験版です。
大きな違いは、Gemini の一次判定案を Ver3 より強く内部スコアへ反映する点です。
...
3. `app/analyzer.py`
   - 判定ロジックの中心です。
   - まずローカルルールで一次判定を作り、Gemini が使える場合は一次判定案と根拠確認を重ね、最後に5区分の判定へ整えます。
   - Ver5 では Gemini 一次判定案の反映を Ver3 より強めています。
...
Ver5 の実験ポイントだけ見たい場合は、`app/analyzer.py` の
`PRIMARY_REVIEW_RISK_WEIGHT` / `PRIMARY_REVIEW_CONFIDENCE_WEIGHT` /
`PRIMARY_REVIEW_RISK_DELTA_CAP` / `PRIMARY_REVIEW_CONFIDENCE_DELTA_CAP` と
`apply_gemini_primary_review` を見ると分かりやすいです。
