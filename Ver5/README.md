...
- Ver5 は Ver3 をベースに、Gemini の一次判定案をより強く取り入れる実験版です
- Gemini 一次判定によるリスク点補正を `0.35 / 最大12pt` から `0.60 / 最大20pt` に強めています
- Gemini 一次判定による確信度補正を `0.30 / 最大10pt` から `0.50 / 最大16pt` に強めています
...
- Ver5 では、その Gemini 一次判定案を Ver3 より強く内部リスク点と確信度へ反映します
...
```powershell
cd "C:\Users\oneuk\OneDrive\Desktop\datamix\01 講義\05_インテグレーションステップ\fake-news-checker\Ver5"
python -m venv .venv
