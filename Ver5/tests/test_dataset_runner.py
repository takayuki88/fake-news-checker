...
def test_run_dataset_records_passed_gemini_preflight(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.dataset_runner.load_dataset", lambda path: {"cases": cases})
    monkeypatch.setattr("app.dataset_runner.build_settings", lambda use_gemini: Settings(gemini_api_key="dummy-key"))
    monkeypatch.setattr("app.dataset_runner.run_gemini_preflight", fake_run_gemini_preflight)
    monkeypatch.setattr("app.dataset_runner.analyze_case_with_retry", fake_analyze_case_with_retry)
...
def test_create_evaluation_output_paths_uses_timestamp_and_expected_names() -> None:
    ...
    output_paths = create_evaluation_output_paths(
        ...
        csv_base_name="Ver5_real_article_dataset_v2_with_predicted_verdict_attention_score",
    )
    ...
    assert output_paths["csv"].name == "Ver5_real_article_dataset_v2_with_predicted_verdict_attention_score.csv"
